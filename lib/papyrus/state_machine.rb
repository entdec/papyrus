  ...
  module StateMachine
    ...
    included do
      ...
      state_machine.events.map(&:name)
                   ...
                   .each do |event_name|
        generator.send(:define_method, event_name) do |object, options = {}|
        end
      end
      ...
      state_machine do
        ...
        after_transition any => any do |record, transition|
          Papyrus::EventRecord.create(transition.event, record, data_store: Papyrus.papyrus_datastore.deep_dup, state_machine: true)
        end
