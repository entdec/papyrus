...
module Papyrus
  module Concerns
    module MetadataScoped
      ...
      included do
        scope :visible, -> { instance_exec(&Papyrus.config.visible_scope) }
      ...
      def add_metadata
        self.metadata ||= {}
        unless Papyrus.config.metadata_fields.empty?
          Papyrus.config.metadata_fields.each do |field, data|
            if data[:current]
        ...
        instance_exec(&Papyrus.config.add_metadata)
      end
    end
  end
end
